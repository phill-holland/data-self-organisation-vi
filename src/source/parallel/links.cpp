...
void organisation::parallel::links::copy(::organisation::genetic::links **source, int source_size)
{
    ...
    sycl::int4 *hostLinks = sycl::malloc_host<sycl::int4>(settings.max_word_count * settings.max_chain * settings.host_buffer, qt);    
    int *hostLinkCount = sycl::malloc_host<int>(settings.max_word_count * settings.host_buffer, qt);
    
    if((hostLinks != NULL)&&(hostLinkCount != NULL))
    {
        memset(hostLinks, -1, sizeof(sycl::int4) * settings.max_word_count * settings.max_chain * settings.host_buffer);
        memset(hostLinkCount, 0, sizeof(int) * settings.max_word_count * settings.host_buffer);
        
        sycl::range num_items{(size_t)settings.clients()};

        int client_offset = settings.max_word_count * settings.max_chain;
        int client_index = 0;
        int dest_index = 0;
        int index = 0;
        ...
        for(int source_index = 0; source_index < source_size; ++source_index)
        {
            ...
            {            
                ...
                if(links->get(temp,i))
                {
                    if((temp.x != -1)||(temp.y != -1)||(temp.z != -1))
                    {
                        ...
                        hostLinkCount[(c_count / settings.max_chain) + (index * settings.max_word_count)] += 1;
                    }
                    ++c_count;
            ...
            if(index >= settings.host_buffer)
            {
                ...
                events.push_back(qt.memcpy(&deviceLinkCount[dest_index * settings.max_word_count], hostLinkCount, sizeof(int) * settings.max_word_count * index));        
